...
본 프로젝트는 **MPC(Model Predictive Control)** 를 도입하여 미래 풍속을 예측하고 선제적으로 최적 피치각을 결정함으로써, **발전 효율 손실 없이 DEL을 최대 80% 감소**시키는 것을 목표로 합니다.
...
| 항목 | 상태 | 내용 |
|------|:----:|------|
| 풍력터빈 수학 모델 구현 | ✅ 완료 | Cp(λ,β) · RK4 적분 · 피치 액추에이터 |
| MPC 제어기 구현 | ✅ 완료 | MPPT + DEL 비용함수 · SLSQP 최적화 |
| PI 제어기 구현 | ✅ 완료 | 비교 기준 제어기 |
| DEL 계산기 구현 | ✅ 완료 | RMS 피치 변화율 · 누적 피로하중 |
| 3가지 풍속 시나리오 시뮬레이션 | ✅ 완료 | STEP · GUST · VARYING |
| MPC vs PI 성능 비교 | ✅ 완료 | DEL 45~80% 감소 확인 |
| Q3 트레이드오프 분석 | ✅ 완료 | 효율 ↔ 수명 최적 운전점 도출 |
| SCADA 실제 데이터 수집 | ✅ 완료 | 8호기 1년치 10분 단위 데이터 |
| RNN 풍속 예측 모델 연동 | 🔄 진행 중 | 팀 협업 · 모델 학습 중 |
